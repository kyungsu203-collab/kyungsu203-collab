<div align="center">

<img src="https://i.namu.wiki/i/n8-k9izfhbnCSk3HLNZ0tMLIeMO2f9O6zDoAEPHH6ApbSP0uzgopseJJIKCwtR5hOMX6v1CKcvHB-7XLwoHMwpEQhSSn6mZdvWX9lUGCFPJLa2wiQ8gKuImERXFMmAErRHAoV5PJt9o8CcyedF88BQ.webp" width="180">

</div>

# 👋 Hello, I'm Kyungsu

> **AI / Machine Learning Developer**

Python과 Machine Learning을 공부하며  
데이터를 이해하고 AI 기술을 활용한 서비스를 만드는 것을 목표로 하고 있습니다.

---

## 📑 Table of Contents

- [About Me](#about-me)
- [Tech Stack](#tech-stack)
- [Currently Learning](#currently-learning)
- [Projects](#projects)
- [Study](#study)
- [Goals](#goals)
- [Contact](#contact)

---

## 👨‍💻 About Me

안녕하세요. **AI와 Machine Learning을 공부하고 있는 개발자 지망생**입니다.

Python을 기반으로 데이터 분석과 머신러닝을 학습하고 있으며,  
Computer Vision과 Deep Learning 분야까지 학습 범위를 넓혀가고 있습니다.

단순히 기술을 사용하는 것에 그치지 않고  
직접 배우고, 만들어보고, 기록하면서 성장하는 것을 목표로 합니다.

- 🔭 Currently studying **AI / Machine Learning**
- 🌱 Learning **Python, Data Analysis, Deep Learning**
- 👁️ Interested in **Computer Vision**
- 🧠 Interested in **AI Applications**
- 📝 Documenting my learning and projects
- 🚀 Building practical AI projects

---

## 🛠 Tech Stack

### Languages

![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white)

### Data Analysis

![Pandas](https://img.shields.io/badge/Pandas-150458?style=flat-square&logo=pandas&logoColor=white)

![NumPy](https://img.shields.io/badge/NumPy-013243?style=flat-square&logo=numpy&logoColor=white)

### Machine Learning

![Scikit-Learn](https://img.shields.io/badge/Scikit--Learn-F7931E?style=flat-square&logo=scikit-learn&logoColor=white)

### Deep Learning & Computer Vision

![TensorFlow](https://img.shields.io/badge/TensorFlow-FF6F00?style=flat-square&logo=tensorflow&logoColor=white)

![OpenCV](https://img.shields.io/badge/OpenCV-5C3EE8?style=flat-square&logo=opencv&logoColor=white)

### Development Tools

![Git](https://img.shields.io/badge/Git-F05032?style=flat-square&logo=git&logoColor=white)

![GitHub](https://img.shields.io/badge/GitHub-181717?style=flat-square&logo=github&logoColor=white)

![Jupyter](https://img.shields.io/badge/Jupyter-F37626?style=flat-square&logo=jupyter&logoColor=white)

![PyCharm](https://img.shields.io/badge/PyCharm-000000?style=flat-square&logo=pycharm&logoColor=white)

---

## 📚 Currently Learning

현재 **AI / Machine Learning 과정**을 중심으로 공부하고 있습니다.

```text
Python
   ↓
Data Analysis
   ↓
Machine Learning
   ↓
Deep Learning
   ↓
Computer Vision
   ↓
AI Application
